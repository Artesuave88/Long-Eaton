...
function _page($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    ...
    $$renderer2.push(`<!--]--> <p class="mt-5 max-w-2xl text-lg leading-8 text-ink/72">${escape_html(data.event.excerpt)}</p> <div class="surface-card mt-8 grid gap-4 p-6 sm:grid-cols-2 lg:grid-cols-4"><div><p class="text-xs font-semibold uppercase tracking-[0.2em] text-brand-700">Date</p> <p class="mt-2 text-base font-semibold text-ink">${escape_html(formatDisplayDate(data.event.date))}</p></div> <div><p class="text-xs font-semibold uppercase tracking-[0.2em] text-brand-700">Start</p> <p class="mt-2 text-base font-semibold text-ink">${escape_html(data.event.startTime ?? data.event.time)}</p></div> <div><p class="text-xs font-semibold uppercase tracking-[0.2em] text-brand-700">Location</p> <p class="mt-2 text-base font-semibold text-ink">${escape_html(data.event.location)}</p></div> `);
    if (data.event.price) {
      $$renderer2.push("<!--[0-->");
      $$renderer2.push(`<div><p class="text-xs font-semibold uppercase tracking-[0.2em] text-brand-700">Entry</p> <p class="mt-2 text-base font-semibold text-ink">${escape_html(data.event.price)}</p></div>`);
    } else {
      $$renderer2.push("<!--[-1-->");
    }
    $$renderer2.push(`<!--]--></div> <div class="mt-8"><h2 class="font-display text-2xl text-ink">About this event</h2> <div class="mt-4 space-y-5 text-base leading-8 text-ink/75"><!--[-->`);
    ...
    $$renderer2.push(`<!--]--></div></div> <div class="surface-card mt-8 p-6"><h2 class="font-display text-2xl text-ink">Timing details</h2> <div class="mt-5 grid gap-4 sm:grid-cols-2"><div class="inset-panel p-4"><p class="text-xs font-semibold uppercase tracking-[0.2em] text-brand-700">Starts</p> <p class="mt-2 text-base font-semibold text-ink">${escape_html(data.event.startTime ?? data.event.time)}</p></div> `);
    if (data.event.endTime || data.event.approximateReturnTime) {
      $$renderer2.push("<!--[0-->");
      $$renderer2.push(`<div class="inset-panel p-4"><p class="text-xs font-semibold uppercase tracking-[0.2em] text-brand-700">${escape_html(data.event.approximateReturnTime ? "Approximate return" : "Ends")}</p> <p class="mt-2 text-base font-semibold text-ink">${escape_html(data.event.approximateReturnTime ?? data.event.endTime)}</p></div>`);
    } else {
      $$renderer2.push("<!--[-1-->");
    }
    ...
    if (data.event.volunteerRun) {
      $$renderer2.push("<!--[0-->");
      $$renderer2.push(`<div class="surface-card mt-8 p-6"><h2 class="font-display text-2xl text-ink">Organiser</h2> <p class="mt-4 text-base leading-8 text-ink/75">${escape_html(data.event.organiser)}. The event is run entirely by volunteers.</p></div>`);
    } else {
      $$renderer2.push("<!--[-1-->");
    }
    $$renderer2.push(`<!--]--> `);
    if (data.event.relatedDates?.length) {
      $$renderer2.push("<!--[0-->");
      $$renderer2.push(`<div class="surface-card mt-8 p-6"><h2 class="font-display text-2xl text-ink">Related dates</h2> <p class="mt-4 text-sm leading-7 text-ink/72">Car boot dates listed alongside the carnival.</p> <div class="mt-5 grid gap-3 sm:grid-cols-2"><!--[-->`);
      const each_array_1 = ensure_array_like(data.event.relatedDates);
      for (let $$index_1 = 0, $$length = each_array_1.length; $$index_1 < $$length; $$index_1++) {
        let item = each_array_1[$$index_1];
        $$renderer2.push(`<div class="inset-panel p-4"><p class="text-xs font-semibold uppercase tracking-[0.2em] text-brand-700">${escape_html(item.title)}</p> <p class="mt-2 text-base font-semibold text-ink">${escape_html(formatRelatedDate(item.date, item.dateLabel))}</p> `);
        if (item.note) {
          $$renderer2.push("<!--[0-->");
    ...
    $$renderer2.push(`<!----> <div class="surface-card p-6"><h2 class="font-display text-2xl text-ink">Planning your visit</h2> <p class="mt-4 text-sm leading-7 text-ink/72">`);
    if (isCarnival) {
      $$renderer2.push("<!--[0-->");
