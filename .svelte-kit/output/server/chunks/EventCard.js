...
function EventCard($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    ...
    $$renderer2.push(`<!----> <div class="p-5"><div class="flex flex-wrap items-center gap-2"><span class="chip">${escape_html(event.category)}</span> <span class="text-sm text-ink/55">${escape_html(formatDisplayDate(event.date))}</span></div> <h3 class="mt-4 font-display text-[1.7rem] leading-tight text-ink">${escape_html(event.title)}</h3> <p class="mt-2 text-sm font-medium text-ink/60">${escape_html(event.location)} • ${escape_html(event.time)}</p> `);
    ...
    $$renderer2.push(`<!--]--> <p class="mt-4 text-sm leading-7 text-ink/72">${escape_html(event.excerpt)}</p> <a${attr("href", `/events/${event.slug}`)} class="mt-5 inline-flex items-center text-sm font-semibold text-brand-700 hover:text-brand-900">See details</a></div></article>`);
