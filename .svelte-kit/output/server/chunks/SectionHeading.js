...
function SectionHeading($$renderer, $$props) {
  ...
  if (eyebrow) {
    $$renderer.push("<!--[0-->");
    $$renderer.push(`<p class="mb-3 text-sm font-semibold uppercase tracking-[0.28em] text-brand-700">${escape_html(eyebrow)}</p>`);
  } else {
    $$renderer.push("<!--[-1-->");
  }
  $$renderer.push(`<!--]--> <h2 class="font-display text-3xl uppercase leading-tight text-ink sm:text-4xl">${escape_html(title)}</h2> `);
  if (copy) {
    $$renderer.push("<!--[0-->");
